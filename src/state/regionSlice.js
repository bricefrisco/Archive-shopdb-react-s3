...
export const fetchRegion = (name, server) => (dispatch) => {
  ...
  fetch(`${process.env.REACT_APP_BACKEND}/region?server=${server}&name=${name}`)
    .then(parseResponse)
    .then((response) => {
...
export const fetchRegionPlayers = (name, server) => (dispatch, getState) => {
  ...
  fetch(
    `${process.env.REACT_APP_BACKEND}/region/players?server=${server}&name=${name}&page=${page}`
  )
    .then(parseResponse)
    .then((response) => {
      dispatch(
...
export const fetchRegionChestShops = (name, server, tradeType) => (
  ...
  fetch(
    `${process.env.REACT_APP_BACKEND}/region/chest-shops?server=${server}&name=${name}&page=${page}&tradeType=${tradeType}`
  )
    .then(parseResponse)
    .then((response) => {
      dispatch(
