...
export const fetchPlayer = (name) => (dispatch) => {
  ...
  fetch(`${process.env.REACT_APP_BACKEND}/player?name=${name}`)
    .then(parseResponse)
    .then((response) => {
...
export const fetchPlayerRegions = (name) => (dispatch, getState) => {
  ...
  fetch(
    `${process.env.REACT_APP_BACKEND}/player/regions?name=${name}&page=${page}`
  )
    .then(parseResponse)
    .then((response) => {
      dispatch(
...
export const fetchPlayerChestShops = (name, tradeType) => (
  ...
  fetch(
    `${process.env.REACT_APP_BACKEND}/player/chest-shops?name=${name}&page=${page}&tradeType=${tradeType}`
  )
    .then(parseResponse)
    .then((response) => {
      dispatch(
